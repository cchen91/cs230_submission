...
dicomDir = np.loadtxt(dirListPath+'dicomPath_cor.txt', dtype=str, delimiter = " ")
niftiDir = np.loadtxt(dirListPath+'niftiPath_cor.txt', dtype=str, delimiter = " ")

num_patient = len(niftiDir)

for i in range(num_patient):
    pathtmp = desPath+'cor_'+str(i+1)+'/'
    os.mkdir(pathtmp)
    ...
    np.save(pathtmp+'x_cor_'+str(i+1)+'.npy', x)
    np.save(pathtmp+'y_cor_'+str(i+1)+'.npy', y)
    np.save(pathtmp+'yalt_cor_'+str(i+1)+'.npy', yalt)
